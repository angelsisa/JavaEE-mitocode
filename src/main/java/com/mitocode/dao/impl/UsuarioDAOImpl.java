...
public class UsuarioDAOImpl implements IUsuarioDAO, Serializable {
	...
	@Override
	public List<Usuario> listaUsuarios(String us) {
		List<Usuario> lista = new ArrayList<>();
		try {
			Query query = em.createQuery("SELECT u FROM Usuario u "
					+ "where u.usuario like:nickname");
			query.setParameter("nickname", "%"+us+"%");
	...
	@Override
	public String verificarPass(String us, String claveHash) {
		List<Usuario> lista = new ArrayList<>();
		String btnGuardar= "";
		try {
			Query query = em.createQuery("FROM Usuario u where u.usuario =?1 and u.contrasena =?2");
			query.setParameter(1, us);
			query.setParameter(2, claveHash);
